...
VSLite.transition <- function(syear, eyear, phi, Te, Pr, year_transition, k_lin, k_sig, 
  ...
  gT <- std.ramp.transition(Te, T1, T2, years, year_transition, k_lin, k_sig)
  
  # Soil moisture growth response:
  gM <- std.ramp.transition(M, M1, M2, years, year_transition, k_lin, k_sig)
